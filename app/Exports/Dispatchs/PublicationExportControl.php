...
class PublicationExportControl implements FromQuery, WithEvents, WithProperties, WithHeadings, WithChunkReading, WithMapping
{
    ...
    {
        return [
            'Nota', 'Ordem', 'Rubrica', 'Municipio', 'Material', 'Data Despacho', 'Data Atribuição', 'Data Vencimento', 'Status', 'Empresa', 'Usuario'
        ];
    }
    ...
    public function map($row): array
    {
        ...
        return [
            $row->Note->note,
            isset($row->Note->Orders) ? implode(chr(10), $row->Note->Orders->filter(function ($order) {
