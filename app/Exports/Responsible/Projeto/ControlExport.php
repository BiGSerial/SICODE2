...
class ControlExport implements FromQuery, WithEvents, WithChunkReading, WithMapping, WithProperties, WithHeadings
{
    ...
    public function map($row): array
    {
        return [
            ...
            $row->rubrica,
            $row->lexp,
            $row->orders->isNotEmpty() &&  $row->orders->fisrt()->operations->ioNotEmpty() ? $row->orders->last()->operations->first()->cenTrab : '',
            ...
            $row->appproval && $row->approval->reclaims->isNotEmpty() ? $row->approval->reclaims->last()->created_at->diffInDays(now()) : 'SEM DEFINI',
            $row->emResolucao,
            $row->status,
        ];
    }
