...
class ListToPayment extends Component
{
    use WithPagination;
    ...
    public function baseQuery()
    {
        return External::query()
            ...
            ->where(function ($w) {
                $w->whereIn('status', ['AGUARDANDO_PAGAMENTO']);
            })
            ...
            ->with([
                ...
                'PoolPayments:id,user_id,external_id,pool_id,criacao_pedido,status_pedido,valor,fi_fbv0',
                'PoolPayments.user:id,name',
            ]);
    }
    ...
    {
        return view('livewire.services.oexterno.list-to-payment', [
            'lists' => $this->lists,
        ]);
    }
}
