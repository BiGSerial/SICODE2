...
use App\Traits\WildcardFormatter; // <-- usa seu trait

class ListUndefined extends Component
{
    use WithPagination;
    use WildcardFormatter; // <-- habilita formatWithWildcard()
    ...
    public function baseQuery()
    {
        return External::query()
            ...
            ->where(function ($w) {
                $w->whereNotIn('status', ['AGUARDANDO_PAGAMENTO', 'AGUARDANDO_ORGAO', 'AGUARDANDO_TAXA'])
                  ->orWhereNull('status');
            })
    ...
    {
        return view('livewire.services.oexterno.list-undefined', [
            'lists' => $this->lists,
        ]);
    }
}
