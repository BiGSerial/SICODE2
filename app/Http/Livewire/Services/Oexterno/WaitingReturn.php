...
class WaitingReturn extends Component
{
    ...
    public function getListsProperty()
    {
        ...
        $query->whereHas('external', function ($q) {
            $q->whereHas('reclaims', function ($qr) {
                $qr->orderBy('reclaims.created_at', 'desc')
                    ->where(function ($q2) {
                        $q2->where('completed', false)
                            ->orWhereRaw('completed_at < externals.updated_at');
                    });
            });
            ...
            $query->where(function ($q) {
                $q->where('note', 'like', "%{$this->search}%")
                    ->orWhereHas('orders', function ($q2) {
                        $q2->where('ordem', 'like', "%{$this->search}%");
                    });
            });
        }
        ...
        if ($this->multinotas) {
            $query->where(function ($q) {
                $q->whereIn('note', $this->multinotas)
                    ->orWhereHas('orders', function ($q2) {
                        $q2->whereIn('ordem', $this->multinotas);
                    });
            });
        }
        ...
        return $query
            ->with(['external.reclaims' => function ($q) {
                $q->orderBy('reclaims.created_at', 'desc')
                    ->with('production')
                    ->first();
            }])
