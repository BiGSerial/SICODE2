...
class BlockEvaluator
{
    ...
    public function evaluate(Note $note)
    {
        ...
        //     return $this->res(self::HOLD_YELLOW, false, '<strong>Obra fora de status de construção</strong> Atual: ' . $note->nstats . ' <p>Entre em contato com o engenheiro da sua região</p>');
        // }

        // // 4) Obra fora de status de construção
        // if ($note->type_note == 1 && $this->centerjobIsSetAndNotCONS($note->centerjob)) {
        //     return $this->res(self::HOLD_YELLOW, false, '<strong>Obra fora de status de construção</strong> Atual: ' . $note->centerjob . '<p>Entre em contato com o engenheiro da sua região</p>');
        // }
