                ...
                <div class="modal-body transfer-modal__body">
                    @if ($transfer_view)
                        ...
                        <div class="transfer-modal__grid">
                            <div>
                                <label for="transfer_company" class="form-label transfer-modal__label">Empresa</label>
                                <select id="transfer_company" class="form-select transfer-modal__control"
                                    wire:model="company_transfer_id">
                                    <option value="">Todas as empresas habilitadas</option>
                                    @foreach ($company_list as $company)
                                        <option wire:key="transfer-company-{{ $company->id }}" value="{{ $company->id }}">
                            ...
                            <div>
                                <label for="transfer_search" class="form-label transfer-modal__label">Buscar usuário</label>
                                <div class="transfer-modal__search">
                                    <i class="ri-search-line"></i>
                                    <input id="transfer_search" type="text" class="form-control transfer-modal__control"
                        ...
                        <div class="mb-3">
                            <label for="transfer_user" class="form-label transfer-modal__label">Novo titular</label>
                            <select id="transfer_user" class="form-select transfer-modal__control"
                                ...
                                @foreach ($user_list as $user)
                                    ...
                                    @endphp
                                    <option wire:key="transfer-user-{{ $user->id }}" value="{{ $user->id }}">
                                        {{ $name }} - {{ $companyName }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            <label for="transfer_reason" class="form-label transfer-modal__label">Motivo <span
                                    class="text-danger fw-bold">*</span></label>
                            <textarea id="transfer_reason" class="form-control transfer-modal__control" wire:model.defer="user_transfer_info"
