...
<div>
        ...
        <div class="card-body">
            <div class="row">
                ...
                <div class="col-6 d-flex justify-content-end">
                    ...
                    @livewire('components.filter.filter', ['myKey' => 'region', 'sendFilter' => 'city', 'model' => 'App\Models\City', 'column' => 'regiao', 'filter' => 'Regiao', 'group_filter' => 'partner', 'values' => 'regiao', 'direction' => 'ASC', 'query' => ''], key('region'))
                    @livewire('components.filter.filter', ['myKey' => 'city', 'sendFilter' => '', 'model' => 'App\Models\City', 'column' => 'cidade', 'filter' => 'Municipio', 'group_filter' => 'partner', 'values' => 'municipio', 'direction' => 'ASC', 'query' => ''], key('city'))
    ...
    @if (!$lists->count())
        <div class="text-center my-5 py-3">
            <h3>NENHUMA OBRA JUSTICADA ENCONTRADA</h3>
        </div>
    @endif
    ...
    @if ($lists->count())
        ...
        <div class="card mb-2 edp-bg-gray">
            ...
            <div class="card-header edp-bg-seoweedgreen-100 text-white">
                <div class="row">
                    <div class="col">
                        <h4 class="my-0">Justificativas a Avaliar</h4>
                    </div>
                ...
                <table class="table table-sm table-condensed table-striped table-hover">
                    <thead>
                        <tr>
                            ...
                            <th scope="col" class="text-center align-middle">Venc Tácito</th>
                            <th scope="col" class="text-center align-middle">Justificado Em</th>
                    ...
                    <tbody class="table-group-divider">
                        @foreach ($lists as $index => $viability)
                            ...
                            <tr wire:key="viability-{{ $viability->id }}"
                                wire:dblclick="$emitTo('responsible.actions.tacit-justify','getResponse', {{ $viability->id }})"
        ...
        return !(strpos($order->statusSist, 'ENT') === 0 || strpos($order->statusSist, 'ENC') === 0);
                                ...
                                <td class="text-center align-middle fw-bold">
                                    {{ Carbon::parse($viability->sended_at)->format('d/m/Y H:i:s') }}
                                </td>
                                <td class="text-center align-middle fw-bold">
                                    <span>
                                        {{ $dueDate->format('d/m/Y H:i:s') }}
                                    </span>
                                </td>
                                <td class="text-center align-middle text-primary fw-bold">
                                    {{ Carbon::parse($viability->tacit_at)->format('d/m/Y H:i:s') }}
                                </td>
                                <td class="text-center align-middle text-danger fw-bold">
                                    {{ Carbon::parse($viability->Justification->justified_at)->format('d/m/Y H:i:s') }}
                                </td>
                                ...
                                <td class="text-center align-middle text-danger fw-bold">
                                    {{ Carbon::parse($viability->Justification->justified_at)->diffForHumans(['short' => false, 'syntax' => Carbon::DIFF_ABSOLUTE]) }}
                                </td>
    ...
    {{-- Livewire Components --}}
    @livewire('responsible.actions.tacit-justify', key('justiFyResponsibleTavit'))
</div>
