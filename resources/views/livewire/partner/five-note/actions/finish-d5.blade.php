        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
                ...
                <div class="modal-body fivefx-body">
                    @if (!empty($five))
                        ...
                        <div class="mb-3">
                            ...
                            {{-- @if ($files->count())
                                <div class="fivefx-filelist">
                                    @foreach ($files as $f)
                                        <div class="fivefx-fileitem">
                                            <i class="ri-file-line me-2"></i>
                                            <span
                                                class="text-truncate">{{ $f->original_name ?? $f->stored_name }}</span>
                                            <span
                                                class="fivefx-meta ms-auto">{{ number_format(($f->size ?? 0) / 1024, 1, ',', '.') }}
                                                KB</span>
                                        </div>
                                    @endforeach
                                </div>
                            @else
                                <div class="fivefx-empty">Sem arquivos referenciais.</div>
                            @endif --}}
                        ...
                        <div class="mb-3">
                            @livewire('files.evidence.upload-evidence', ['five' => $five, 'type' => 'POS_OBRA', 'origin' => $origin])

                            {{-- Avisos finais --}}
                            <div class="fivefx-info mt-3">
                                <ul class="mb-0">
                                    <li>Ao encerrar, registraremos a data/hora de conclusão.</li>
                                    <li>As evidências anexadas ficarão vinculadas a esta D5.</li>
                                </ul>
                            </div>
                        @else
                                            ...
                                            <div>
                                                <div class="fivefx-completion-name">
                                                    ...
                                                    @if ($production->User?->email)
                                                        <span class="teams-contact-icon" title="Entrar em contato"
            ...
            <div class="modal-footer fivefx-footer">
                ...
                <button class="btn btn-success fivefx-btn"
                    @if (!empty($five)) wire:click="finishD5({{ $five->id }})" @endif
                    @disabled($five?->name === null || $five?->name == '')>
