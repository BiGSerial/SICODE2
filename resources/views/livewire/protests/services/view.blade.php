...
<div>
    ...
    {{-- ==== ENCERRAMENTO / PARECER FINAL ==== --}}
    @if (!$medProtest->Assignments?->where('user_id', auth()->id())?->first()->completed)
        <div class="modern-card mt-3">
            <div class="modern-card-body">
                <div class="d-flex justify-content-between align-items-center mb-2">
                    <span class="modern-card-title d-flex align-items-center gap-2">
                        <i class="ri-verified-badge-line"></i> Encerrar Atividade / Parecer Final
                    </span>
                    @php
                        $needsEvidence = (bool) ($medProtest->needsEvidence ?? false);
                        $hasEvidence = $medProtest->evidenceFiles && $medProtest->evidenceFiles->count() > 0;
                    @endphp
                    @if ($needsEvidence)
                        <span class="badge {{ $hasEvidence ? 'bg-success' : 'bg-warning text-dark' }}">
                            <i class="ri-attachment-2"></i>
                            {{ $hasEvidence ? 'Evidências anexadas' : 'Evidências obrigatórias pendentes' }}
                        </span>
                    @endif
                </div>
                ...
                <div class="row g-3">
                        ...
                        <div class="form-floating">
                            ...
                            <div class="d-flex justify-content-between mt-1 small text-muted">
                                <span>
                                    ...
                                    Este texto constará no Relatório Técnico (laudo) da medida.
                                </span>
                                <span>
                                    <span id="parecerCount">{{ mb_strlen($conclusion ?? '') }}</span> / 2000
                                </span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-md-3 d-flex flex-column justify-content-between">
                        <div
                            class="alert {{ $needsEvidence && !$hasEvidence ? 'alert-warning' : 'alert-light border' }} py-2 mb-3">
                            <div class="d-flex align-items-start gap-2">
                                <i class="ri-shield-check-line mt-1"></i>
                                <div class="small">
                                    @if ($needsEvidence && !$hasEvidence)
                                        É obrigatório anexar pelo menos um arquivo antes de encerrar.
                                    @else
                                        Revise as informações antes de concluir. Esta ação registrará o laudo.
                                    @endif
                                </div>
                            </div>
                        </div>
                        ...
                        <button
                            class="btn btn-lg w-100 {{ $needsEvidence && !$hasEvidence ? 'btn-secondary' : 'btn-primary' }}"
                            wire:click="finishMedProtest" @disabled($needsEvidence && !$hasEvidence)>
    ...
    @push('scripts')
        <script>
            // contador simples sem dependências
            document.addEventListener('livewire:init', () => {
                const txt = document.getElementById('parecerFinal');
                const cnt = document.getElementById('parecerCount');
                if (txt && cnt) {
                    const update = () => cnt.textContent = (txt.value || '').length;
